...
def calculate_event_differences1(df):
    ...
    all_events_diff = pd.DataFrame(columns= list(columnas_numericas.columns))
    ...
    for i in range(0, total_filas - 1, 2):
        ...
        resta_filas_df = pd.DataFrame(resta_filas, columns=list(columnas_numericas.columns))

        # Agregar la resta al nuevo DataFrame all_events_diff
        #all_events_diff = all_events_diff.append(resta_filas, ignore_index=True)

        #all_events_diff = pd.concat([all_events_diff, resta_filas_df], axis =0, ignore_index=True)
